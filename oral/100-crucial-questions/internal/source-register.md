...
### F. Board-sufficiency and 129-item empirical mapping audit

**Audit files:**
- `oral/100-crucial-questions/reviews/010-board-sufficiency-audit.md`
- `oral/100-crucial-questions/reviews/010-greek-oral-129-mapping-audit.md`

**Sources used:**
- Attempt 010 and its internal coverage/map files;
- all 129 items in the curated `orestispsom/Psych/src/data/oral.js` bank;
- `ORAL_QUALITY_AUDIT.md`;
- `MCQ_QUALITY_AUDIT.md`;
- `src/data/sos.js` and `highYieldPsychiatryTables.js` as precision/recognition checks.

Purpose:
- test the stronger hypothesis that deep mastery of the 100 answer packages should plausibly suffice for the general Greek Adult Psychiatry specialist-certification examination, not merely give broad topic coverage;
- determine whether any of the 129 curated oral items still requires a new main slot;
- separate coherent oral-answer content from precise board facts/short factual probes.

129-item mapping result against Attempt 010:
- 44/129 `COVERED_DIRECTLY` by a main or visible follow-up;
- 55/129 `COVERED_WITHIN_ANSWER` and therefore mandatory hidden answer content;
- 30/129 `FACT_ANCHOR`, best protected through the board-fact layer;
- 0/129 deliberately excluded as lower-priority/not forced.

Design consequence:
- no main-slot reallocation is currently required on the evidence of the curated Greek oral bank;
- `internal/board-fact-anchors.yml` is now a formal layer beneath the 100;
- exact criteria, durations, key calculations, classic distinctions and drug-specific facts must be retrievable without bloating the visible question list;
- older exact values remain source material rather than current truth until verified;
- one additional visible suicide-risk/disorder-pattern probe was justified and added in Attempt 011.

Canonical-gap decisions generated by this audit:
- `risk.intimate_partner_violence` approved for canonical promotion as IMPORTANT answer coverage, without a dedicated main;
- `neuropsychiatry.apathy` approved for canonical promotion because apathy versus depression has direct Greek oral provenance and should not be represented only indirectly through depression/neuroanatomy.

### G. Current major examination / curriculum frameworks — secondary completeness check
...
- whether the finding affects MAIN questions, FOLLOW_UP prompts, ANSWER_COVERAGE, BOARD_FACT anchors, or no change.
...
- The current 100 main slots are provisionally frozen unless stronger Greek-exam evidence exposes a real adult-board gap.
...
- The final learning package should combine oral core + must-not-miss answer coverage + verified board-fact anchors + genuine examiner pivots.
- Exact source-derived board facts must not be mistaken for current truth until the appropriate diagnostic, prescribing, regulatory or legal source has been verified.
