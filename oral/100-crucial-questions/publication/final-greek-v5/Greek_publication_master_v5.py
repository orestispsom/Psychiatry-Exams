...
SRC = Path(os.environ.get('GREEK_SRC','oral/100-crucial-questions/answers/revision-v5-el'))
OUT = Path(os.environ.get('GREEK_OUT','build_greek'))
OUT.mkdir(parents=True, exist_ok=True)
SOURCE_COMMIT=os.environ.get('SOURCE_COMMIT','2e4feabfe56df22411a8b7704b50058bcaf421b2')
...
def read_questions():
    ...
    if len(files) != 100: raise RuntimeError(f'Expected exactly 100 sources, found {len(files)}')
...
def assemble(cover,toc,body,starts,followups):
    ...
    final=OUT/'The_100_Crucial_Questions_in_Psychiatry_v5_GREEK_COMPLETE_A4.pdf'; out.save(final,garbage=4,deflate=True,clean=True); out.close()
...
if __name__=='__main__':
    ...
    (OUT/'GREEK_V5_COMPLETE_book_manifest.json').write_text(json.dumps(manifest,ensure_ascii=False,indent=2),encoding='utf-8')
    (OUT/'GREEK_V5_COMPLETE_QA_report.json').write_text(json.dumps(report,ensure_ascii=False,indent=2),encoding='utf-8')
